...
def build_2d():
    cells = [
        md("# Clinical Validation of 2D Mammography AI — CBIS-DDSM\n"
           ...
           "density, lesion type.\n"
           "**Model:** a linear probe — a frozen ImageNet-pretrained backbone "
           "with a logistic-regression head fit on the CBIS-DDSM training "
           "split (swap in any model by writing one `ImageClassifier`).\n"
        ...
        md("## 4 · Build the model — a linear probe\n"
           "Reliable ready-made mammography classifiers are scarce on public "
           "model hubs and frequently broken. The robust, reproducible "
           "alternative is a **linear probe**: a frozen ImageNet-pretrained "
           "backbone used purely as a feature extractor, with a "
           "logistic-regression head fit on the CBIS-DDSM **training** split.\n"
           "\n"
           "The backbone is never updated — only the linear head is fit, and "
           "only on training data — so the test split is still validated as a "
           "black box. (`HFImageClassifier` stays available for any well-formed "
           "HuggingFace checkpoint.)"),
        code("from mammoval.models import LinearProbeClassifier\n"
             "\n"
             "model = LinearProbeClassifier(backbone='resnet50')\n"
             "# Fit the logistic head on the TRAINING split; the backbone stays\n"
             "# frozen. Use limit=<int> for a quick pass, None for the full split.\n"
             "model.fit(ds_train, limit=None)\n"
             "print('fitted:', model.name, '| head trained on', model.n_train,\n"
             "      'images |', model.feature_dim, 'features')"),
        ...
        code("from mammoval.report import build_report\n"
             ...
             "        'The model is a linear probe (frozen ImageNet backbone + '\n"
             "        'logistic head) - a deliberately simple baseline; its absolute '\n"
             "        'numbers are a lower bound, the validation methodology is the point.',\n"
             "    ])\n"
             "HTML(open(path, encoding='utf-8').read())"),

        md("## Notes & next steps\n"
           "* **Stronger model.** Try another backbone "
           "(`LinearProbeClassifier('efficientnet_b0')`), or write one "
           "`ImageClassifier` subclass for a fully-trained model — the "
...
def build_3d():
    cells = [
        ...
        md("## 4 · Build the 3D classifier\n"
           ...
           "`SliceAggregatorClassifier` scores a sample of slices with a 2D "
           "classifier and pools them (top-k mean).\n"
           "\n"
           "For the 2D base we use the same **linear probe** as the 2D "
           "notebook, fit on CBIS-DDSM. On Kaggle, also **+ Add Input** the "
           "*\"CBIS-DDSM Breast Cancer Image Dataset\"* (awsaf49) — the cell "
           "auto-detects it. Applying a 2D mammography classifier to "
           "tomosynthesis slices is a reasonable, runnable baseline; a native "
           "3D detector would be stronger (see the FROC notes below)."),
        code("import os, glob\n"
